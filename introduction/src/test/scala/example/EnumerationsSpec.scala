...
class EnumerationsSpec extends FlatSpec with Matchers {
  "Value assigns a numerical value to fields" should "be" in {
  ...
  "You can create an enumeration with your own index and your own Strings" should "be" in {
  ...
  "Enumerations can be declared in one line if you are merely setting variables to Value" should "be" in {
    object Planets extends Enumeration {
      val Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto =
        Value
    }
  ...
  "Enumerations can be declared with a string value only" should "be" in {
    object GreekPlanets extends Enumeration {
  ...
  "You can extend the Enumeration by extending the Value class" should "be" in {
    object Planets extends Enumeration {
      val G = 6.67300E-11

      class PlanetValue(val i: Int,
                        val name: String,
                        val mass: Double,
                        val radius: Double)
