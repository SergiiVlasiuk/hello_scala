...
object p28 {
  ...
  def lsortFreq[A](ls: List[List[A]]): List[List[A]] = {
    val lsToLength = ls map { _.length } sortWith { _ < _ }
    println(s"lsToLength: $lsToLength")
    val encoded: Seq[(Int, Int)] = encode(lsToLength)
    println(s"encoded: $encoded")
    val swapped: Seq[(Int, Int)] = encoded map { _.swap }
    println(s"swapped: $swapped")
    val freqs: Map[Int, Int] = Map(swapped:_*)
    println(s"Map: $freqs")
