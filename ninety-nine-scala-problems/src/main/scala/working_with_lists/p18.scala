...
object p18 {
  ...
  def sliceTailRecursive[A](start: Int, end: Int, ls: List[A]): List[A] = {
    ...
    def sliceR(count: Int, curList: List[A], result: List[A]): List[A] =
      (count, curList) match {
        case (_, Nil)                     =>
          println(s"case (_, Nil)                     =>curList: $curList; result: $result")
          result.reverse
        case (c, h :: tail) if end <= c   =>
          println(s"case (c, h :: tail) if end <= c   => c: $c; curList: $curList; result: $result")
          result.reverse
        case (c, h :: tail) if start <= c =>
          println(s"case (c, h :: tail) if start <= c => c: $c; h: $h; tail: $tail; curList: $curList; result: $result")
          sliceR(c + 1, tail, h :: result)
        case (c, _ :: tail)               =>
          println(s"case (c, _ :: tail)               => c: $c; tail: $tail; curList: $curList; result: $result")
          sliceR(c + 1, tail, result)
      }
    sliceR(0, ls, Nil)
  }
  ...
  // Functional.
  def sliceFunctional2[A](s: Int, e: Int, ls: List[A]): List[A] =
    ls.slice(s, s + (e - (s max 0)))
}
